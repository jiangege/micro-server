...
class MicroServer {
  ...
  #startHttpServer() {
    ...
    locales(app, {
      dirs: [path.join(this.config.projectDir, "/locales")],
      ...this.config.locales,
    });
      ...
      try {
        ...
        if (e instanceof Joi.ValidationError) {
          ctx.body = {
            ...
            data: {
              code: 400,
              message: app.__(reqBody.locale, "sys.bad_request"),
              details: e.details,
            },
          };
        }
          ...
          data: {
            ...
            message:
              this.config.env === "production"
                ? app.__(reqBody.locale, "sys.internal_server_error")
                : e.message,
    ...
    router.all(
      ...
      async (ctx) => {
        ...
        if (!this.#verifyCallRequest(service, logic, func)) {
          throw new Error(ctx.__(`sys.invalid_request`));
        }
        const reqBody = this.#getReqBody(ctx);
        ...
        const result = await this.#callFunc(service, logic, func, {
          ...reqBody,
          from: "api",
          __: (...args) => app.__(reqBody.locale, ...args),
        });
    ...
    io.on("connection", (socket) => {
      socket.on("call", async (data, callback) => {
        try {
          ...
          if (!this.#verifyCallRequest(service, logic, func)) {
            throw new Error(this.app.__(reqBody.locale, `sys.invalid_request`));
          }

          const result = await this.#callFunc(service, logic, func, {
            ...reqBody,
            from: "socketio",
            __: (...args) => this.app.__(reqBody.locale, ...args),
          });
